...
export default function AppPage() {
  ...
  const [selectedPhotoLocation, setSelectedPhotoLocation] = useState<{ lat: number; lng: number } | null>(null)
  const [capturedPhotoDataUrl, setCapturedPhotoDataUrl] = useState<string | null>(null)
  const [selectedPhotoView, setSelectedPhotoView] = useState<any | null>(null)
  ...
  useEffect(() => {
    if (selectedChat && lastChatIdRef.current !== selectedChat.id && messages.length > 0) {
      scrollToBottom()
  ...
  const handleImageUpload = async (file?: File) => {
    ...
    try {
      ...
      const uploadRes = await fetch(getApiUrl("/api/location-chat/upload"), {
        method: "POST",
        body: formData,
      })
  ...
  const takeCameraPhoto = async () => {
    try {
      ...
      if (error.message !== "User cancelled photos app") {
        setImageError("Failed to capture photo")
      }
    }
  }
  ...
  const selectGalleryPhoto = async () => {
    try {
      ...
      if (error.message !== "User cancelled photos app") {
        setImageError("Failed to select photo")
      }
    }
  }
  ...
  const handleLocationPhotoUpload = async (file?: File) => {
    const uploadLocation = selectedPhotoLocation || location
    if (!uploadLocation || !user) return

    let fileToUpload = file

    if (!fileToUpload && capturedPhotoDataUrl) {
      fileToUpload = dataUrlToFile(capturedPhotoDataUrl, `photo-${Date.now()}.jpg`)
    }

    if (!fileToUpload) {
      setImageError("No photo selected")
      return
    }
    ...
    try {
      ...
      formData.append("latitude", uploadLocation.lat.toString())
      formData.append("longitude", uploadLocation.lng.toString())
      if (proximityChat?.id) {
        formData.append("chatId", proximityChat.id)
      }

      const uploadRes = await fetch(getApiUrl("/api/location-photos"), {
        method: "POST",
        body: formData,
      })
  ...
  const handlePhotoClick = (lat: number, lng: number) => {
    setSelectedPhotoLocation({ lat, lng })
    setActiveTab("photos")
    fetchLocationPhotos(undefined, lat, lng)
  }
      ...
      <header className="sticky top-0 z-50 glass border-b border-border/50 backdrop-blur-xl">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="w-10 h-10 rounded-xl bg-secondary/60 animate-pulse" />
          </div>
        </div>
      </header>
          ...
          >
            <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
              <button
                ...
                className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-secondary/50 transition-colors flex-shrink-0"
              >
    ...
    <>
      ...
      <div className="min-h-[100dvh] safe-inset flex flex-col bg-background">
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-6 space-y-6 pb-32">
                  ...
                  <div>
                    ...
                    {selectedPhotoLocation && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Viewing photos near {selectedPhotoLocation.lat.toFixed(4)}, {selectedPhotoLocation.lng.toFixed(4)}
                      </p>
                    )}
                  </div>
                  <Button
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {locationPhotos.map((photo, index) => (
                      <button
                        ...
                        className="glass rounded-xl overflow-hidden border border-border/50 hover:border-cyan-500/50 transition-all cursor-pointer"
                      >
                ...
                {selectedPhotoLocation ? (
                  <div className="mb-3 text-xs text-cyan-400 bg-cyan-500/10 rounded-lg px-3 py-2 flex items-center justify-between">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setSelectedPhotoLocation(null)}
                  ...
                  <div className="mb-3 text-xs text-amber-400 bg-amber-500/10 rounded-lg px-3 py-2 flex items-center gap-2">
                    <Target className="w-4 h-4 flex-shrink-0" />
                    <span>Tap anywhere on the map to select photo location</span>
                  </div>
                )}
                ...
                {capturedPhotoDataUrl && (
                  <div className="mb-4">
                    <img 
                      ...
                      alt="Preview" 
                      className="w-full h-48 object-contain rounded-xl bg-secondary/40"
                    />
                  </div>
                )}
          ...
          {selectedPhotoView && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm safe-inset" onClick={() => setSelectedPhotoView(null)}>
              <div className="max-w-4xl w-full mx-4" onClick={(e) => e.stopPropagation()}>
                <div className="relative">
                  <Button
                    ...
                    className="absolute -top-12 right-0 rounded-full bg-white/10 hover:bg-white/20 h-10 w-10"
                  >
                    <X className="w-6 h-6" />
                  </Button>
                  <img
                    src={selectedPhotoView.photo_url}
                    alt={selectedPhotoView.caption || "Location photo"}
                    className="w-full max-h-[80vh] object-contain rounded-xl"
                  />
                  <div className="mt-4 bg-secondary/95 rounded-xl p-4 border border-border/50">
                    {selectedPhotoView.caption && (
                      <p className="text-base mb-3">{selectedPhotoView.caption}</p>
                    )}
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">
                          {selectedPhotoView.user?.display_name || selectedPhotoView.user?.username}
                        </p>
        ...
        <Lightbox
          ...
          index={lightboxIndex}
          slides={locationPhotos.map(photo => ({ src: photo.photo_url }))}
          plugins={[Zoom]}
        />
      </div>
    </>
